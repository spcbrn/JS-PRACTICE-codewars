...
//     let set = [...sig];
//     for (let i = 4; i <= n; i++) {
//         set.push(set.slice(set.length - 3).reduce((a, b) => a + b));
//     }
//     return set;
// }
...
//recursive version:
const tribonacci = (sig, n) => {
    if (sig.length === 3 && n <= 3) return [...sig.splice(0, n)];
    return n > 3
        ? tribonacci([...sig, sig.slice(sig.length - 3).reduce((a, b) => a + b)], n - 1)
        : sig;
}

console.log([1, 2, 3].splice(0).reduce((a, b) => a + b))
